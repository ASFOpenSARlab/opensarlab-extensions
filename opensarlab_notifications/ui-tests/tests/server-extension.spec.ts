...
test('should store state between reloads', async ({ page }) => {
  await Promise.all([
    page.waitForRequest(
      (request) =>
        request.url().search(/opensarlab-notifications\/hello/) >= 0 &&
        request.method() === 'GET'
    ),
    page.waitForRequest(
      (request) =>
  ...
  expect(
    await page
      .frame({ url: /\/opensarlab-notifications\/public\/index.html/ })
      .waitForSelector(
        'text=This content is served from the opensarlab_notifications server extension.'
      )
  ).toBeTruthy();
});
